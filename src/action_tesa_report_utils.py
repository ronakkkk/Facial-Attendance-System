...
from .queries import select_from_person_roster, get_department_from_db, run_select_query, select_from_holiday
...
def get_shift(empid,emp_shift_id, emp_curr_shift_timestamp, timestamp,timestamp_format="%Y-%m-%d"):
    ...
    if datetime.strptime(timestamp, timestamp_format) >= datetime.strptime(emp_curr_shift_timestamp, SHIFT_TIMESTAMP_FORMAT) or get_date_and_time(timestamp, timestamp_format)[0] >= get_date_and_time(emp_curr_shift_timestamp, SHIFT_TIMESTAMP_FORMAT)[0]:
        # print(1)
...
def get_department(empid,emp_dep_id,emp_curr_dept_timestamp,timestamp,timestamp_format="%Y-%m-%d"):
    ...
    final_dept_id = None
    if datetime.strptime(timestamp, timestamp_format) >= datetime.strptime(emp_curr_dept_timestamp, DEPARTMENT_TIMESTAMP_FORMAT) or get_date_and_time(timestamp, timestamp_format)[0] >= get_date_and_time(emp_curr_dept_timestamp, DEPARTMENT_TIMESTAMP_FORMAT)[0]:
...
def get_compay_holiday_list(year):
    ...
    person_db_connection = create_connection(CONFIG['DB']['person_db'])
    holidays_from_db = select_from_holiday(person_db_connection, htype = "Company", year = year, return_dc= True)
    person_db_connection.close()
...
def get_department_holiday_list(year, deptid = None):
    ...
    person_db_connection = create_connection(CONFIG['DB']['person_db'])
    holidays_from_db = select_from_holiday(person_db_connection, htype = "Department", deptid= deptid, year= year, return_dc= True)
    person_db_connection.close()
