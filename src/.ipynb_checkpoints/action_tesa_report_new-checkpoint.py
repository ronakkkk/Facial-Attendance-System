...
#     print("\nresult1: ",res[0])
    ...
    if k==len(res)-1: 
        ...
        #     no_log_status = "Mispunched"
        no_log_status = "Mispunched"
        #TODO INCLUDE ENTRY OR EXIT TIME IN REPORT
        res3_no_log=[[emp_id,emp_curr_dept_hod,emp_name,emp_curr_shift_name,emp_curr_dept_name,start_date_copy,res1['INTIME'][0],start_date_copy,None,None,None,no_log_status,day_type]]
        if DEBUG:
            print("checkpoint8")
        return res1,res2,pd.DataFrame(res3_no_log,columns=res3_names)
   ...
   #get daily work status  
    if day_type!="Working Day":
        ...
        if not emp_curr_shift==DYNAMIC_SHIFT_ID:
            ...
            elif string_to_seconds(rows[-1][column_order.index('A.outtime')],IN_TIME_FORMAT)<emp_curr_shift_end_time-RELAXATION_TIME_EXIT:
...
def extract_action_tesa_report_data(person_db_connection, user_map, start_date=None, end_date=None,location=None, report_type = ["cam","workhour","daily"]):
...
#     for ul, un in zip(user_list, user_name):
    for ul,un in user_map.items():
        res,res2,res3 = extract_action_tesa_report_data(person_db_connection,
                                                   ...
                                                   end_date=end_date,
                                                   location=None, report_type = report_type)
        if return_cam and res.shape[0] > 0:
            results.append(res)
        if return_workhour and res2.shape[0] > 0:
            results2.append(res2)
...
def extract_action_tesa_monthly_report(person_db_connection, user_map, month=None,year=None,location=None):
    ...
    if len(res3)<1:
        print("person:", user_map.keys() ,"has no log in this month:", month)
        return res3
    ...
    entry_no_monthly_status_dict = defaultdict(lambda: monthly_status_list)
    ...
    for row in res3.values:
        ...
        res4_dict[entry_no]=[row[i] for i in column_indices4]
        
        if late_by != None:
            entry_no_late_by_dict[entry_no].append(late_by)

        entry_no_monthly_status_dict[entry_no][month_days_list.index(row[entry_date_ind])] = day_monthly_status_map[row[status_ind]]
        # except Exception as e:
        #     print(e, len(res3.values))
        #     print(row)
        #     return
        
    for entry_no, monthly_status in entry_no_monthly_status_dict.items():    
        entry_no_monthly_analysis_dict[entry_no].extend([monthly_status.count(monthly_analysis_map[i]) for i in monthly_analysis_columns])
    ...
    for entry_no,values in entry_no_late_by_dict.items():
        ...
        late_days_count = entry_no_monthly_analysis_dict[entry_no][monthly_analysis_columns.index("LATE COMER")]
        ...
        entry_no_late_penalty_dict[entry_no] = late_penalty
        
        
    for entry_no in res4_dict.keys():
        res4_dict[entry_no].extend(entry_no_monthly_status_dict[entry_no])
        res4_dict[entry_no].extend(entry_no_monthly_analysis_dict[entry_no])
        res4_dict[entry_no].append(entry_no_late_penalty_dict[entry_no])
...
#     for ul, un in zip(user_list, user_name):
    for ul,un in user_map.items():
        res = None
        # try:
        res = extract_action_tesa_monthly_report(person_db_connection, {ul:un}, month = month ,year = year,location = location)
        # except Exception as e:
        #     print(e)
        #     print('ul un', ul, un)
